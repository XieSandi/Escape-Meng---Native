...
'use strict'; {
    ...
    function UpdateAdmobPlusRequest(requestProperty,
        propertyValue) {
    ...
    function SetInitConfigureOptions(id, pubID, privacyPolicy, showFree, showConsent, debug, debugLocation) {
    ...
    async function TestId() {
        if (self["cordova"]["platformId"] === "android") return self.C3AdUtilsMD5(self["device"]["uuid"]).toUpperCase();
...
'use strict'; {
    ...
    class C3BannerAd {
        constructor(id, size, position, resolver) {
            ...
            if (!size) return Failure(resolver,
        ...
        async Hide(resolver) {
            if (!this._ad) return Failure(resolver, "banner ad not created");
            if (this._adLoading) return Failure(resolver, "banner ad still loading");
            if (!this._adShowing) return Success(resolver, "banner ad not being shown");
            await this._ad["hide"]();
            this._adShowing = false;
            Success(resolver,
                "banner ad hidden")
    ...
    function _AddLoadListeners(resolver) {
        ...
        const onFailLoad = event => {
            ...
            document.removeEventListener(Events.LOAD, onLoad);
            document.removeEventListener(Events.LOAD_FAIL,
                onFailLoad)
        };
...
'use strict'; {
    ...
    class C3InterstitialAd {
        ...
        async Show(resolver) {
            ...
            if (this._adShowing) return Success(resolver, "interstitial ad already shown");
            const show_ad = async () => {
...
'use strict'; {
    ...
    class C3RewardedAd {
        ...
        async Show(resolver) {
            ...
            if (this._adShowing) return Success(resolver, "rewarded ad already shown");
            const show_ad = async () => {
                if (this._loadSuccess) {
                    _AddShowListeners.call(this, resolver);
                    await this._ad["show"]()
    ...
    function _AddShowListeners(resolver) {
        ...
        document.addEventListener(Events.SHOW, onShow);
        document.addEventListener(Events.SHOW_FAIL, onShowFail);
        document.addEventListener(Events.REWARD,
            onReward);
...
'use strict'; {
    ...
    class C3RewardedInterstitialAd {
        ...
        async Show(resolver) {
            if (!this._ad) return Failure(resolver, "rewarded interstitial ad not created");
            if (this._adShowing) return Success(resolver, "rewarded interstitial ad already shown");
            const show_ad = async () => {
                if (this._loadSuccess) {
                    _AddShowListeners.call(this, resolver);
                    await this._ad["show"]()
    ...
    function _AddLoadListeners(resolver) {
        ...
        const onFailLoad = event => {
            this._adLoading = false;
            this.ResolveLoadPromise(false);
            if (resolver) Failure(resolver, "rewarded interstitial ad failed to load");
            document.removeEventListener(Events.LOAD, onLoad);
            document.removeEventListener(Events.LOAD_FAIL, onFailLoad)
        };
        document.addEventListener(Events.LOAD, onLoad);
    ...
    function _AddShowListeners(resolver) {
        ...
        const onShowFail = event => {
            this._adShowing = false;
            if (resolver) Failure(resolver, "rewarded interstitial ad failed to show");
            document.removeEventListener(Events.SHOW, onShow);
            document.removeEventListener(Events.SHOW_FAIL, onShowFail);
            document.removeEventListener(Events.REWARD, onReward);
            document.removeEventListener(Events.DISMISS, onDismiss)
...
'use strict'; {
    ...
    const ANDROID = self["cordova"] ? self["cordova"]["platformId"] === "android" : false;
    const UNKNOWN = ANDROID ? 0 : 0;
    const NOT_REQUIRED = ANDROID ? 1 : 2;
    const REQUIRED = ANDROID ? 2 : 1;
    const OBTAINED = ANDROID ? 3 : 3;
    class C3Consent {
        constructor() {
            this._adPersonalization = "UNKNOWN";
            this._inEEAorUnknown = "";
            this._consentStatusIndex = 0;
            this._consentStatusString =
                "UNKNOWN";
            this._trackingStatusString = "not-determined";
            this._hasConsentForm = false
        }
        async Reset() {
            if (!!C3AdUtils.ConfigurationOptions.debug) await self["consent"]["reset"]()
        }
        ...
        async GetTrackingAuthorizationStatus() {
            ...
            const idfaPlugin = self["cordova"]["plugins"]["idfa"];
            const idfaInfo = await idfaPlugin.getInfo();
            if (!idfaInfo.trackingLimited) return this._trackingStatusString = "authorized";
        ...
        async RequestIDFA(resolver) {
            if (ANDROID) return Success(resolver,
                "not-determined");
            if (IOS) {
                await _UpdateRequestLocationInEEAorUnknown.call(this);
                if (this._inEEAorUnknown === "true") await this.RequestTrackingAuthorization();
                else {
        ...
        async RequestInfoUpdateForTesting(debug,
            debugLocation, resolver) {
            const ret = await _RequestInfoUpdateForTesting.call(this, debug, debugLocation, error => {
                if (error) Failure(resolver, error.message);
                else Failure(resolver, "requestInfoUpdate error")
            });
            return ret
        }
        async RequestInfoUpdateForTestingSkipErrors(debug, debugLocation, resolver) {
        ...
        async SuccessResponse(resolver) {
            await this.GetTrackingAuthorizationStatus();
            Success(resolver, `${this._consentStatusString}&&${this._trackingStatusString}&&${this._inEEAorUnknown}`)
        }
    ...
    async function _UpdateConsentStatus() {
        ...
        switch (this._consentStatusIndex) {
            case REQUIRED: {
                return this._consentStatusString =
                    "REQUIRED"
            }
            case NOT_REQUIRED: {
    ...
    async function _UpdateRequestLocationInEEAorUnknown() {
        if (ANDROID) switch (this._consentStatusIndex) {
            case REQUIRED: {
        ...
        if (IOS) {
            const gdprApplies = await _GetValueFromNativeStorage.call(this,
                "IABTCF_gdprApplies", "getInt");
    ...
    async function _GetValueFromNativeStorage(name, method) {
        return new Promise((resolve, reject) => {
            self["NativeStorage"][method](name, res => resolve(res), error => resolve(1))
        })
    }
    async function _RequestInfoUpdateForTesting(debug, debugLocation, onError) {
        ...
        try {
            if (!!debug) await self["consent"]["requestInfoUpdate"]({
...
'use strict'; {
    ...
    const rhex = n => {
        let s = "";
        for (let j = 0; j < 4; j++) s += hex_chr[n >> j * 8 + 4 & 15] + hex_chr[n >> j * 8 & 15];
        return s
    };
    const hex = x => {
...
'use strict'; {
    ...
    addHandler("CreateInterstitialAdvert", async (id, debug, fn) => {
        if (interstitialAd) return C3AdUtils.Success(fn,
            "interstitial ad already created");
    ...
    addHandler("ShowInterstitialAdvert", async fn => {
        if (!interstitialAd) return C3AdUtils.Failure(fn, "interstitial ad not created");
        interstitialAd.Show(fn)
    });
    addHandler("CreateVideoAdvert", async (id, debug, fn) => {
        if (videoAd || rewardedAd) return C3AdUtils.Success(fn, "video ad already created");
        if (!await C3AdUtils.StartAdmobPlus(fn, debug)) return;
        const adPersonalization = await C3Consent.GetAdPersonalization();
    ...
    addHandler("CreateRewardedAdvert", async (id, debug, fn) => {
        if (rewardedAd || videoAd) return C3AdUtils.Success(fn, "rewarded ad already created");
        if (!await C3AdUtils.StartAdmobPlus(fn, debug)) return;
        const adPersonalization =
            await C3Consent.GetAdPersonalization();
    ...
    addHandler("CreateRewardedInterstitialAdvert", async (id, debug, fn) => {
        if (rewardedInterstitialAd) return C3AdUtils.Success(fn, "rewarded interstitial ad already created");
        if (!await C3AdUtils.StartAdmobPlus(fn, debug)) return;
        const adPersonalization = await C3Consent.GetAdPersonalization();
    ...
    addHandler("ShowRewardedInterstitialAdvert", async fn => {
        if (!rewardedInterstitialAd) return C3AdUtils.Failure(fn, "rewarded interstitial ad not created");
        rewardedInterstitialAd.Show(fn)
    });
    addHandler("Configure", async (id, pubID, privacyPolicy, showFree, showConsent, debug, debugLocation, showOnStartUp, fn) => {
        const firstCall = !C3AdUtils.HasInitConfigurationOptions();
        C3AdUtils.SetInitConfigureOptions(id,
            pubID, privacyPolicy, showFree, showConsent, debug, debugLocation);
        if (!await C3AdUtils.StartAdmobPlus(fn, debug)) return;
        if (firstCall) await C3Consent.Reset();
        if (!await C3Consent.RequestInfoUpdateForTesting(debug, debugLocation, fn)) return;
        if (firstCall)
    ...
    addHandler("RequestConsent", async fn => {
        api["Configure"](C3AdUtils.ConfigurationOptions.id,
            C3AdUtils.ConfigurationOptions.pubID, C3AdUtils.ConfigurationOptions.privacyPolicy, C3AdUtils.ConfigurationOptions.showFree, "always", C3AdUtils.ConfigurationOptions.debug, C3AdUtils.ConfigurationOptions.debugLocation, false, fn)
    });
    ...
    addHandler("TagForChildDirectedTreatment", async (label, fn) => {
        if (!await C3AdUtils.StartAdmobPlus(fn)) return;
        try {
            C3AdUtils.UpdateAdmobPlusRequest("tagForChildDirectedTreatment", label === 1 ? true : false);
            C3AdUtils.Success(fn, "")
        } catch (error) {
    ...
    addHandler("TagForUnderAgeOfConsent", async (label, fn) => {
        if (!await C3AdUtils.StartAdmobPlus(fn)) return;
        try {
            C3AdUtils.UpdateAdmobPlusRequest("tagForUnderAgeOfConsent", label === 1 ? true : false);
            C3AdUtils.Success(fn,
                "")
        } catch (error) {
            C3AdUtils.Failure(fn, "invalid label")
    ...
    addHandler("StatusUpdate", async (debug, debugLocation, fn) => {
        if (!await C3AdUtils.StartAdmobPlus(fn, debug)) return;
        await C3Consent.RequestInfoUpdateForTestingSkipErrors(debug, debugLocation, fn);
...
'use strict'; {
    ...
    addHandler("CreateVideoAdvert", async (...a) => {
        const [data, fn] = getArgument("CreateVideoAdvert",
            a);
        await sleep(50);
...
'use strict'; {
    ...
    const HANDLER_CLASS = class MobileAdvertHandler extends self.DOMHandler {
        constructor(iRuntime) {
            ...
            this.AddRuntimeMessageHandlers([handler("CreateBannerAdvert"), handler("ShowBannerAdvert"), handler("HideBannerAdvert"), handler("CreateInterstitialAdvert"), handler("ShowInterstitialAdvert"), handler("CreateVideoAdvert"), handler("ShowVideoAdvert"),
        ...
        async _CallMethod(name, data) {
            let api;
            if (self["cordova"]) api = self.C3MobileAdvertsAPI.real;
            else if (USE_EMULATOR) api =
